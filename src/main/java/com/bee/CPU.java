...
public class CPU {
  ...
  // constructor
  public CPU(String name, BlockingQueue<Job> incoming,
    ...
    while (this.completed.size() < this.numJobs) {
      try {
        ...
        if (this.verbosePrinting) {
          System.out.println("Next job on " + this.name + ": " +
              next.toString());
        }
        /*
        ...
        if (next.getLength() > 0) {
          completed.add(next);
          if (this.verbosePrinting) {
            System.out.println("Completed on " + this.name + ": " +
                next.toString());
            System.out.println("We've run " + this.completed.size() + " jobs.");
          } else if (this.completed.size() % (this.numJobs / 10) == 0) {
            System.out.println("CPU " + name + " has completed " +
                this.completed.size() + " jobs.");
          }
        }
